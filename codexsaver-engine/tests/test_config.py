...
def test_deepseek_enabled_defaults_to_true(tmp_path):
    assert is_deepseek_enabled(str(tmp_path / "missing.json")) is True


def test_set_deepseek_enabled(tmp_path):
    config_path = tmp_path / "config.json"
    report = set_deepseek_enabled(False, str(config_path))
    assert report["deepseek_enabled"] is False
    assert is_deepseek_enabled(str(config_path)) is False
