...
class TestCodexSaverEngine:
    ...
    def test_delegate_task_respects_disabled_deepseek(self, monkeypatch):
        monkeypatch.setattr("codexsaver.engine.is_deepseek_enabled", lambda: False)
        with patch("codexsaver.engine.ProviderClient") as mock_client:
            result = self.engine.delegate_task({
                "instruction": "为工具函数添加单元测试",
                "files": [],
            })
        assert result["route"] == "codex"
        assert result["status"] == "needs_codex"
        assert "DeepSeek" in result["decision"]["reason"]
        mock_client.assert_not_called()

    def test_codex_can_explicitly_approve_bounded_delegation(self):
        result = self.engine.delegate_task({
        ...
        assert result["delegation_reason"].startswith("调用 DeepSeek 的原因")
