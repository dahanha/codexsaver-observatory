...
# DeepSeek routing is enabled by default for backwards compatibility with
# existing installations.  The dashboard can persist this as a top-level
# setting in ~/.codexsaver/config.json.
DEFAULT_DEEPSEEK_ENABLED = True
...
def load_compression_config(config_path: str | None = None) -> Dict[str, Any]:
    ...
    enabled = bool(compression.get("enabled", DEFAULT_COMPRESSION_CONFIG["enabled"]))
...
def save_compression_config(
    *,
    ...
    return {
        "config_path": path,
        "compression": load_compression_config(path),
    }
...
def is_deepseek_enabled(config_path: str | None = None) -> bool:
    """Return whether automatic DeepSeek delegation is currently enabled."""
    config = load_config(config_path)
    value = config.get("deepseek_enabled", DEFAULT_DEEPSEEK_ENABLED)
    return bool(value)


def set_deepseek_enabled(enabled: bool, config_path: str | None = None) -> Dict[str, Any]:
    """Persist the automatic DeepSeek delegation switch."""
    config = load_config(config_path)
    config["deepseek_enabled"] = bool(enabled)
    path = save_config(config, config_path)
    return {
        "config_path": path,
        "deepseek_enabled": is_deepseek_enabled(path),
    }
